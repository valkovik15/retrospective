...
module Mutations
  class ToggleReadyStatusMutation < Mutations::BaseMutation
    ...
    def resolve(id:)
      ...
      unless allowed_to?(:ready_toggle?, membership, context: { membership: membership }, with: API::MembershipPolicy)
        return { errors:
          { full_messages: ['Unauthorized to perform this action'] } }
      end

      if membership.update(ready: !membership.ready)
        RetrospectiveSchema.subscriptions.trigger('membership_updated', { board_slug: membership.board.slug },
                                                  membership)
