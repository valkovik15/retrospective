...
module Mutations
  class AddActionItemMutation < Mutations::BaseMutation
    ...
    def resolve(attributes:)
      ...
      unless allowed_to?(:create?, action_item, context: { user: context[:current_user], board: board })
        return { errors:
          { full_messages: ['Unauthorized to perform this action'] } }
      end

      if action_item.save
        RetrospectiveSchema.subscriptions.trigger('action_item_added', { board_slug: board.slug }, action_item)
